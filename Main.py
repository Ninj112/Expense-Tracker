...
class ExpenseTracker(QMainWindow):
    ...
    def create_budget_section(self):
        ...
        self.set_budget_button = QPushButton("Hello " + userName + ", Click here to view your account")
        self.set_budget_button.setFont(QFont("Arial", 14))
        self.set_budget_button.setMinimumHeight(60)
        ...
        """)
        self.set_budget_button.clicked.connect(self.openAccountLayout)
    ...
    def create_recent_purchases_section(self):
        ...
        self.expense_table = QTableWidget(len(self.expenses), 5)  # 4 columns: Description, Amount, Category, Action
        self.expense_table.setHorizontalHeaderLabels(["Description", "Amount", "Category", "Date", "Delete"])
        ...
        for row, array in enumerate(self.expenses):
            self.expense_table.setItem(row, 0, QTableWidgetItem(str(array[0])))  # Description
            self.expense_table.setItem(row, 1, QTableWidgetItem(str(array[1])))  # Amount
            self.expense_table.setItem(row, 2, QTableWidgetItem(str(array[2])))  # Category
            self.expense_table.setItem(row, 3, QTableWidgetItem(str(array[3])))  # Date

            remove_button = QPushButton("Remove")
            remove_button.clicked.connect(partial(self.remove_expense, row))
            self.expense_table.setCellWidget(row, 4, remove_button)
    ...
    def remove_expense(self, row):
        ...
        if 0 <= row < len(self.expenses):
            ...
            amount = 0 # we are looping here to update the total amount
            for array in self.expenses:
                amount += array[1]
            self.total_amount = amount
            ...
            # updating the budget after the amount have changed
            self.update_budget_status()
